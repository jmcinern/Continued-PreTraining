...
block_size = 2056 # training example size
...
nce_dataset_2056_chunks = nce_tokenized_dataset.map(group_texts, 
                                                    batched=True, 
                                                    # attn padding not important for CPT
                                                    remove_columns=["attention_mask"] 
                                                    )
dail_dataset_2056_chunks = dail_tokenized_dataset.map(group_texts, 
                                                      batched=True,
                                                      remove_columns=["attention_mask"]
                                                      )
...
mixed_dataset = concatenate_datasets([
    nce_dataset_2056_chunks,
    dail_dataset_2056_chunks
]).shuffle(seed=42)
# now load base model
'''
...
trainer = Trainer(
    model=model,
    args=training_args,
    train_dataset=nce_dataset_2056_chunks,
...
'''
# then English
trainer.train_dataset = dail_dataset_2056_chunks
